#!/usr/bin/env python
"""
Script para verificar y completar referencias de cobranzas y pagos
"""
import sys
import os
...
def verificar_completar_referencias():
    """Verificar y completar referencias de cobranzas y pagos"""
    
    with app.app_context():
        print("="*70)
        print("VERIFICANDO Y COMPLETANDO REFERENCIAS")
        print("="*70)
        
        # Verificar qué cobranzas y pagos existen
        print("\n1. Verificando datos existentes...")
        
        cobranzas = Cobranza.query.all()
        pagos = Pago.query.all()
        
        print(f"✓ Cobranzas en DB: {len(cobranzas)}")
        print(f"✓ Pagos en DB: {len(pagos)}")
        
        if len(cobranzas) == 0 and len(pagos) == 0:
            print("⚠ No hay cobranzas ni pagos en la base de datos")
            print("   Primero debes cargar los datos con: python cargar_cobranzas_pagos.py")
            return
        
        # Mostrar cobranzas existentes
        print("\n2. Cobranzas existentes:")
        for cob in cobranzas[:5]:  # Mostrar solo las primeras 5
            cliente = Cliente.query.get(cob.cliente_id)
            print(f"   - {cob.id}: {cliente.nombre if cliente else 'N/A'} - ${cob.monto:,.0f} - Referencia: {cob.referencia or 'N/A'}")
        
        # Mostrar pagos existentes
        print("\n3. Pagos existentes:")
        for pag in pagos[:5]:  # Mostrar solo los primeros 5
            proveedor = Proveedor.query.get(pag.proveedor_id)
            cliente = Cliente.query.get(pag.cliente_id) if pag.cliente_id else None
            print(f"   - {pag.id}: {proveedor.nombre if proveedor else 'N/A'} - ${pag.monto:,.0f} - Cliente: {cliente.nombre if cliente else 'N/A'}")
        
        # Cargar datos de Excel
        print("\n4. Cargando datos de Excel...")
        ...
        print("\n5. Mapeando clientes y proveedores...")
        ...
        # Completar cobranzas
        print("\n6. Completando referencias de cobranzas...")
        ...
        for _, row in df_cobranzas.iterrows():
            ...
            if cobranza:
                ...
                print(f"⚠ Cobranza no encontrada: {cliente_nombre} - ${importe:,.0f}")
        
        # Completar pagos
        print("\n7. Completando referencias de pagos...")
        ...
        for _, row in df_pagos.iterrows():
            ...
            if pago:
                ...
                print(f"⚠ Pago no encontrado: {proveedor_nombre} - ${importe:,.0f}")
...
if __name__ == '__main__':
    verificar_completar_referencias()