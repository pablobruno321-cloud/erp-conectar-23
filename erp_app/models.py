...
class Pago(db.Model):
    ...
    # Nuevas columnas para conciliación
    pedido_id = db.Column(db.Integer, db.ForeignKey('pedidos.id'), nullable=True)
    numero_comprobante = db.Column(db.String(100))
    tipo_comprobante = db.Column(db.String(50))  # 'Factura', 'Recibo', 'Nota de Crédito'
    cliente_id = db.Column(db.Integer, db.ForeignKey('clientes.id'), nullable=True)  # Cliente que paga
